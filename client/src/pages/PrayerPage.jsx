...
import { prayerService, eventService, volunteerService } from '../services/apiService'; // ✅ Added services
...
const PrayerPage = () => {
  ...
  useEffect(() => {
    document.title = "SMC: - Prayers | St. Michael's & All Angels Church | Ifite-Awka";
    fetchPrayers();
    fetchPrayerTeam();
    fetchPrayerMeetings();
  ...
  const fetchPrayers = async (pageNum = 1) => {
    try {
      ...
      if (response.success) {
        const prayersData = Array.isArray(response.data) ? response.data : (response.data?.prayers || []);
        const formattedPrayers = prayersData.map(prayer => new PrayerRequest(prayer));
        setPrayers(prev => pageNum === 1 ? formattedPrayers : [...prev, ...formattedPrayers]);
    ...
    try {
      ...
      if (response.success) {
        ...
        if (volunteersResponse.success) {
          const prayerTeamVolunteers = volunteersResponse.data.filter(volunteer => 
  ...
  const fetchPrayerMeetings = async () => {
    try {
      ...
      if (response.success) {
        const prayerEvents = response.data.filter(event => 
    ...
    try {
      ...
      if (response.success) {
        ...
        const stats = {
          ...
          prayersThisWeek: prayers.filter(p => 
            new Date(p.date) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)
          ).length,
          teamMembers: prayerTeam.length,
          upcomingMeetings: prayerMeetings.filter(m => 
            new Date(m.startTime || m.date) > new Date()
          ).length
        };
  ...
  const getFilteredMeetings = () => {
    ...
    switch (meetingFilter) {
      case 'upcoming':
        return prayerMeetings.filter(meeting => 
          new Date(meeting.startTime || meeting.date) > now
        );
      case 'past':
        return prayerMeetings.filter(meeting => 
          new Date(meeting.startTime || meeting.date) <= now
  ...
  const PrayerTeamTab = () => (
    <div className="space-y-6">
      ...
      {(isAdmin || isModerator) && prayerStats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-white rounded-lg shadow-md p-4 text-center">
            <div className="text-2xl font-bold text-[#FF7E45]">{prayerStats.teamMembers || prayerTeam.length}</div>
            <div className="text-sm text-gray-600">Team Members</div>
          </div>
          <div className="bg-white rounded-lg shadow-md p-4 text-center">
            <div className="text-2xl font-bold text-[#FF7E45]">{prayerStats.totalRequests || prayers.length}</div>
            <div className="text-sm text-gray-600">Total Requests</div>
          </div>
          <div className="bg-white rounded-lg shadow-md p-4 text-center">
            <div className="text-2xl font-bold text-[#FF7E45]">{prayerStats.prayersThisWeek || 0}</div>
            <div className="text-sm text-gray-600">This Week</div>
          </div>
          <div className="bg-white rounded-lg shadow-md p-4 text-center">
            <div className="text-2xl font-bold text-[#FF7E45]">{prayerStats.upcomingMeetings || getFilteredMeetings().length}</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {prayerTeam.length > 0 ? prayerTeam.map((member, index) => (
          <div key={member._id || index} className="bg-white rounded-lg shadow-md p-6 text-center">
            ...
            <h3 className="text-lg font-bold mb-1">{member.name || 'Prayer Team Member'}</h3>
            <p className="text-gray-600 text-sm mb-3">{member.role || 'Prayer Volunteer'}</p>
            <p className="text-gray-700 text-sm mb-4">
              {member.bio || 'Committed to praying for our church community'}
            </p>
            <div className="flex justify-center space-x-2">
              {member.email && (
                <a href={`mailto:${member.email}`} className="text-[#FF7E45] hover:text-[#F4B942]">
                  <i className="fas fa-envelope"></i>
                </a>
              )}
              {member.phone && (
                <a href={`tel:${member.phone}`} className="text-[#FF7E45] hover:text-[#F4B942]">
  ...
  const PrayerResourcesTab = () => (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="w-12 h-12 bg-[#FFF5F0] rounded-full flex items-center justify-center mb-4">
            <i className="fas fa-book-open text-[#FF7E45]"></i>
          </div>
          <h3 className="text-xl font-bold mb-2">Prayer Guides</h3>
          <p className="text-gray-600 mb-4">Step-by-step guides for different types of prayer</p>
          <ul className="space-y-2 text-sm">
            <li><a href="#" className="text-[#FF7E45] hover:text-[#F4B942]">The Lord's Prayer</a></li>
            <li><a href="#" className="text-[#FF7E45] hover:text-[#F4B942]">Prayer for Healing</a></li>
            <li><a href="#" className="text-[#FF7E45] hover:text-[#F4B942]">Intercessory Prayer</a></li>
          </ul>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="w-12 h-12 bg-[#FFF5F0] rounded-full flex items-center justify-center mb-4">
            <i className="fas fa-book-bible text-[#FF7E45]"></i>
          </div>
          <h3 className="text-xl font-bold mb-2">Daily Devotionals</h3>
          <p className="text-gray-600 mb-4">Scripture-based prayer devotionals</p>
          <ul className="space-y-2 text-sm">
            <li><a href="#" className="text-[#FF7E45] hover:text-[#F4B942]">Morning Prayer</a></li>
            <li><a href="#" className="text-[#FF7E45] hover:text-[#F4B942]">Evening Reflection</a></li>
            <li><a href="#" className="text-[#FF7E45] hover:text-[#F4B942]">Weekly Focus</a></li>
          </ul>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="w-12 h-12 bg-[#FFF5F0] rounded-full flex items-center justify-center mb-4">
            <i className="fas fa-tools text-[#FF7E45]"></i>
          </div>
          <h3 className="text-xl font-bold mb-2">Prayer Tools</h3>
          <p className="text-gray-600 mb-4">Practical tools for your prayer journey</p>
          <ul className="space-y-2 text-sm">
            <li><a href="#" className="text-[#FF7E45] hover:text-[#F4B942]">Prayer Journal Template</a></li>
            <li><a href="#" className="text-[#FF7E45] hover:text-[#F4B942]">Family Prayer Guide</a></li>
            <li><a href="#" className="text-[#FF7E45] hover:text-[#F4B942]">Prayer Prompts</a></li>
          </ul>
        </div>
      </div>
      ...
      <div className="bg-gray-50 rounded-lg p-8 mt-8">
        <h3 className="text-2xl font-bold mb-4 text-center">Additional Resources</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <h4 className="font-bold mb-2">Recommended Books</h4>
            <ul className="space-y-1 text-sm text-gray-600">
              <li>• "The Power of Prayer" by E.M. Bounds</li>
              <li>• "Prayer: Experiencing Awe and Intimacy with God" by Timothy Keller</li>
  ...
  const PrayerMeetingsTab = () => (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {getFilteredMeetings().length > 0 ? getFilteredMeetings().map((meeting, index) => (
          <div key={meeting._id || index} className="bg-white rounded-lg shadow-md p-6">
          ...
          <div className="text-center py-12">
            ...
            <button className="btn btn-primary mt-4" onClick={fetchPrayerMeetings}>
            ...
            <div key={prayer.id} className="bg-white rounded-lg shadow-md p-6">
              ...
              <div className="flex justify-between items-center">
                ...
                <button
                  ...
                  className={`flex items-center ${prayer.userPrayed
            ...
            <div className="bg-white rounded-lg shadow-md mb-8">
              <div className="border-b border-gray-200">
                <nav className="flex flex-wrap -mb-px">
                  {[
                    { id: 'requests', label: 'Prayer Requests', icon: 'fa-praying-hands' },
                    { id: 'team', label: 'Prayer Team', icon: 'fa-users' },
                    { id: 'meetings', label: 'Prayer Meetings', icon: 'fa-calendar' },
                    { id: 'resources', label: 'Prayer Resources', icon: 'fa-book-open' }
                  ].map((tab) => (
                    <button
                      key={tab.id}
