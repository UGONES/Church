...
const ProfilePage = ({ user }) => {
  ...
  useEffect(() => {
    document.title =
      "SMC: - Profile | St. Micheal`s & All Angels Church | Ifite-Awka";
  ...
  const fetchUserProfileData = async () => {
    try {
      ...
      if (donationsResponse.status === "fulfilled") {
        setDonations(
          donationsResponse.value.donations || donationsResponse.value,
        );
      } else {
      ...
      if (favoritesResponse.status === "fulfilled") {
        setFavorites(
          favoritesResponse.value.favorites || favoritesResponse.value,
        );
      } else {
  ...
  const handleUpdateCommunicationPrefs = async (preferences) => {
    try {
      const response = await apiClient.put("/user/communication", {
        preferences,
      });
