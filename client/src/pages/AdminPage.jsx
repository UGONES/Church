...
const AdminPage = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      try {
        ...
        setTestimonials(
          testimonialsResponse.testimonials || testimonialsResponse,
        );
  ...
  const handleUpdateSettings = async (settingsData) => {
    try {
      const response = await apiClient.put(
        ADMIN_ENDPOINTS.SETTINGS.UPDATE,
        settingsData,
      );
      setSettings(response.settings || response);
  ...
  const handleCreateMinistry = async (ministryData) => {
    try {
      const response = await apiClient.post(
        ADMIN_ENDPOINTS.MINISTRIES.CREATE,
        ministryData,
      );
      ...
      return {
        success: false,
        message: error.message || "Failed to create ministry",
      };
    }
  };
  ...
  const handleUpdateMinistry = async (ministryId, ministryData) => {
    try {
      const response = await apiClient.put(
        ADMIN_ENDPOINTS.MINISTRIES.UPDATE(ministryId),
        ministryData,
      );
      setMinistries((prev) =>
        prev.map((ministry) =>
          ministry._id === ministryId ? response.ministry : ministry,
        ),
      );
      ...
      return {
        success: false,
        message: error.message || "Failed to update ministry",
      };
    }
  };
  ...
  const handleDeleteMinistry = async (ministryId) => {
    try {
      ...
      return {
        success: false,
        message: error.message || "Failed to delete ministry",
      };
    }
  };
  ...
  const handleCreateTestimonial = async (testimonialData) => {
    try {
      const response = await apiClient.post(
        ADMIN_ENDPOINTS.TESTIMONIALS.CREATE,
        testimonialData,
      );
  ...
  const handleUpdateTestimonial = async (testimonialId, testimonialData) => {
    try {
      const response = await apiClient.put(
        ADMIN_ENDPOINTS.TESTIMONIALS.UPDATE(testimonialId),
        testimonialData,
      );
  ...
  const handleDeleteTestimonial = async (testimonialId) => {
    try {
      await apiClient.delete(
        ADMIN_ENDPOINTS.TESTIMONIALS.DELETE(testimonialId),
      );
      setTestimonials((prev) =>
  ...
  const handleUpdateBlogPost = async (blogId, blogData) => {
    try {
      const response = await apiClient.put(
        ADMIN_ENDPOINTS.BLOG.UPDATE(blogId),
        blogData,
      );
      setBlogPosts((prev) =>
        prev.map((post) => (post._id === blogId ? response.post : post)),
      );
  ...
  const handleUpdateEvent = async (eventId, eventData) => {
    try {
      const response = await apiClient.put(
        ADMIN_ENDPOINTS.EVENTS.UPDATE(eventId),
        eventData,
      );
      setUpcomingEvents((prev) =>
        prev.map((event) => (event._id === eventId ? response.event : event)),
      );
  ...
  const handleCreateSermon = async (sermonData) => {
    try {
      const response = await apiClient.post(
        ADMIN_ENDPOINTS.SERMONS.CREATE,
        sermonData,
      );
  ...
  const handleUpdateSermon = async (sermonId, sermonData) => {
    try {
      const response = await apiClient.put(
        ADMIN_ENDPOINTS.SERMONS.UPDATE(sermonId),
        sermonData,
      );
      setSermons((prev) =>
        prev.map((sermon) =>
          sermon._id === sermonId ? response.sermon : sermon,
        ),
      );
  ...
  const handleUpdateDonation = async (donationId, donationData) => {
    try {
      const response = await apiClient.put(
        ADMIN_ENDPOINTS.DONATIONS.UPDATE(donationId),
        donationData,
      );
      setDonations((prev) =>
        prev.map((donation) =>
          donation._id === donationId ? response.donation : donation,
        ),
      );
  ...
  const handleUpdatePrayerRequest = async (prayerId, prayerData) => {
    try {
      const response = await apiClient.put(
        ADMIN_ENDPOINTS.PRAYER_REQUESTS.UPDATE(prayerId),
        prayerData,
      );
      setPrayerRequests((prev) =>
        prev.map((prayer) =>
          prayer._id === prayerId ? response.prayer : prayer,
        ),
      );
      ...
      return {
        success: false,
        message: error.message || "Failed to update prayer request",
      };
    }
  };
  ...
  const handleDeletePrayerRequest = async (prayerId) => {
    try {
      ...
      return {
        success: false,
        message: error.message || "Failed to delete prayer request",
      };
    }
  };
