...
const SermonsPage = ({ user }) => {
  ...
  useEffect(() => {
    document.title = "SMC: - Sermons | St. Micheal`s & All Angels Church | Ifite-Awka";
   ...
   const handleDeleteSermon = async (sermonId) => {
    ...
    const handleEditSubmit = async (e) => {
        ...
        try {
            let response;
            if (editForm._id) {
                response = await apiClient.put(`/api/sermons/${editForm._id}`, editForm);
            } else {
    ...
    const handleAddToFavorites = async (sermonId) => {
        ...
        try {
            if (favoriteSermons.has(sermonId)) {
                const response = await apiClient.delete(`/api/sermons/favorites/${sermonId}`);
                if (response.success) {
                ...
                const response = await apiClient.post('/api/sermons/favorite', { sermonId });
                if (response.success) {
                ...
                {isAuthenticated && (
                    <button 
                        ...
                        className={`transition-colors ${
                            isFavorite ? "text-[#FF7E45]" : "text-gray-400 hover:text-[#FF7E45]"
                        }`}
                    >
