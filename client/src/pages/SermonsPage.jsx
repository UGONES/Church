...
const SermonsPage = ({ user }) => {
  ...
  useEffect(() => {
    document.title =
      "SMC: - Sermons | St. Micheal`s & All Angels Church | Ifite-Awka";
  ...
  const fetchSermons = async () => {
    try {
      setIsLoading(true);
      setError(null);
      const response = await apiClient.get("/api/sermons");
      if (response.success) {
        setSermons(response.data.map((sermon) => new Sermon(sermon)));
      }
    } catch (error) {
  ...
  const fetchLiveStatus = async () => {
    try {
      const response = await apiClient.get("/api/sermons/live");
      if (response.success) {
  ...
  const fetchCategories = async () => {
    try {
      const response = await apiClient.get("/api/sermons/categories");
      if (response.success) {
  ...
  const fetchUserFavorites = async () => {
    try {
      const response = await apiClient.get("/api/sermons/favorites");
      if (response.success) {
        setFavoriteSermons(new Set(response.data.map((fav) => fav.sermonId)));
      }
    } catch (error) {
  ...
  const fetchSermonStats = async () => {
    try {
      const response = await apiClient.get("/api/sermons/stats");
      if (response.success) {
  ...
  const handleDeleteSermon = async (sermonId) => {
    alert.info("Are you sure you want to delete this sermon?", {
      confirm: async () => {
        try {
          const response = await apiClient.delete(`/api/sermons/${sermonId}`);
          if (response.success) {
            setSermons(sermons.filter((s) => s._id !== sermonId));
    ...
    try {
      ...
      if (editForm._id) {
        response = await apiClient.put(
          `/api/sermons/${editForm._id}`,
          editForm,
        );
      } else {
        response = await apiClient.post("/api/sermons", {
          ...editForm,
      ...
      if (response.success) {
        const savedSermon = new Sermon(response.data);
        if (editForm._id) {
          setSermons(
            sermons.map((s) => (s._id === editForm._id ? savedSermon : s)),
          );
        } else {
    ...
    try {
      if (favoriteSermons.has(sermonId)) {
        const response = await apiClient.delete(
          `/api/sermons/favorites/${sermonId}`,
        );
        if (response.success) {
        ...
        const response = await apiClient.post("/api/sermons/favorite", {
          sermonId,
        });
        if (response.success) {
    ...
    <div className="page">
      ...
      <section className="bg-gradient-to-r from-[#FF7E45] to-[#F4B942] py-12 px-4">
        <div className="container mx-auto text-center text-white">
          ...
          <p className="text-xl max-w-2xl mx-auto">
            Watch or listen to recent messages from our pastors
          </p>
          ...
          {isLoading ? (
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredSermons.map((sermon) => (
                  <SermonCard
                    ...
                    sermon={sermon}
                    isAdmin={isAdmin}
                    isFavorite={favoriteSermons.has(sermon.id)}
              ...
              {filteredSermons.length === 0 && (
                <div className="text-center py-12">
                  <i className="fas fa-sad-tear text-4xl text-gray-400 mb-4" />
      ...
      {showSermonModal && selectedSermon && (
        <SermonModal
          ...
          isFavorite={favoriteSermons.has(selectedSermon.id)}
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center p-4 text-center">
                <div className="mb-4">
                  <div className="relative inline-block">
                    <i className="fas fa-video text-4xl md:text-5xl text-gray-400" />
                    {liveStatus?.isLive && (
          ...
          <div className="bg-gray-800 p-4 md:p-6 rounded-lg shadow-lg h-full">
            ...
            <div className="space-y-4">
              <div className="flex items-start">
                <i className="fas fa-calendar-alt text-[#FF7E45] mt-1 mr-3" />
                <div>
              ...
              <div className="flex items-start">
                <i className="fas fa-clock text-[#FF7E45] mt-1 mr-3" />
                <div>
              ...
              <div className="flex items-start">
                <i className="fas fa-user text-[#FF7E45] mt-1 mr-3" />
                <div>
                  <p className="font-semibold">
                    {liveStatus?.currentSpeaker || "Pastor Michael Johnson"}
                  </p>
                  <p className="text-gray-300 text-sm">
                    "
                    {liveStatus?.currentTopic ||
  ...
  <div className="sermon-card bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
    ...
    <div className="p-4">
      ...
      <div className="mt-3 flex items-center text-sm text-gray-500">
        <span className="mr-3">
          <i className="fas fa-eye mr-1" /> {sermon.views || 0}
        </span>
        <span>
          <i className="fas fa-clock mr-1" /> {sermon.duration || "45:00"}
        </span>
      </div>
    </div>
  </div>
      ...
      <div className="p-6">
        <div className="flex justify-between items-start mb-4">
          ...
          {isAuthenticated && (
            ...
            >
              <i className={`${isFavorite ? "fas" : "far"} fa-heart mr-1`} />
        ...
        <div className="flex flex-wrap gap-3 mb-6">
          ...
          <span className="bg-blue-100 text-blue-800 text-sm font-medium px-3 py-1 rounded">
            <i className="fas fa-eye mr-1" /> {sermon.views || 0} views
          </span>
          <span className="bg-green-100 text-green-800 text-sm font-medium px-3 py-1 rounded">
            <i className="fas fa-thumbs-up mr-1" /> {sermon.likes || 0} likes
          </span>
        </div>
    ...
    <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
      <div className="p-6">
        <h2 className="text-2xl font-bold mb-6">
          {editForm.id ? "Edit Sermon" : "Add New Sermon"}
        </h2>
        ...
        <form onSubmit={onSubmit}>
          ...
          <div className="flex justify-end space-x-3">
            ...
            >
              {editForm.id ? "Update" : "Create"} Sermon
            </button>
          </div>
        </form>
      </div>
