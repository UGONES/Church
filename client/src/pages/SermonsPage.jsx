...
const SermonsPage = () => {
  ...
  useEffect(() => {
    document.title =
      "SMC: - Sermons | St. Michael's & All Angels Church | Ifite-Awka";
    fetchSermons();
    fetchLiveStatus();
    fetchCategories();
    if (isAuthenticated) {
  ...
  const fetchUserFavorites = async () => {
    try {
      ...
      setFavoriteSermons(
        new Set(response.map((fav) => fav.sermonId || fav._id)),
      );
    } catch (error) {
    ...
    alert.info("Are you sure you want to delete this sermon?", {
      confirm: async () => {
        try {
          ...
          setSermons(
            sermons.filter((s) => s._id !== sermonId && s.id !== sermonId),
          );
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      alert.success(
        `Sermon ${editForm._id ? "updated" : "created"} successfully!`,
      );
    } catch (error) {
    ...
    <div className="page">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredSermons.map((sermon) => (
                  <SermonCard
                    ...
                    isAdmin={isAdmin}
                    isFavorite={
                      favoriteSermons.has(sermon._id) ||
                      favoriteSermons.has(sermon.id)
                    }
      ...
      {showSermonModal && selectedSermon && (
        <SermonModal
          ...
          isFavorite={
            favoriteSermons.has(selectedSermon._id) ||
            favoriteSermons.has(selectedSermon.id)
          }
