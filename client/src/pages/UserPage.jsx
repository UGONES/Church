...
const MyRSVPsPage = () => {
  ...
  useEffect(() => {
    document.title =
      "SMC: - RSVPs | St. Micheal`s & All Angels Church | Ifite-Awka";
  ...
  const fetchRSVPs = async () => {
    try {
      ...
      alert.error(
        err.message || "Failed to load your events. Please try again.",
      );
    } finally {
      setLoading(false);
    }
        ...
        confirm: async () => {
          try {
            ...
            const response = await apiClient.post(
              `${API_ENDPOINTS.CANCEL_RSVP}${eventId}`,
            );
            if (response.success) {
              fetchRSVPs();
            ...
            alert.error(
              err.message || "Failed to cancel RSVP. Please try again.",
            );
          } finally {
  ...
  const handleAddToCalendar = async (eventId, eventTitle) => {
    try {
      setActionLoading(true);
      const response = await apiClient.post(
        `${API_ENDPOINTS.ADD_TO_CALENDAR}${eventId}`,
      );
      if (response.success) {
        alert.success(
          `"${eventTitle}" has been added to your calendar successfully!`,
        );
      } else {
      ...
      setError(
        err.message || "Failed to add event to calendar. Please try again.",
      );
      alert.error(
        err.message || "Failed to add event to calendar. Please try again.",
      );
            ...
            {pastEvents.length > 5 && (
              <div className="mt-6 text-center">
                <button
                  ...
                  onClick={() => {
                    /* Implement pagination or view more logic */
                  }}
                >
