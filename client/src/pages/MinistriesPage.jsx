...
const MinistriesPage = () => {
  ...
  const fetchMinistries = async () => {
    try {
      ...
      if (response.success) {
        const ministriesData = response.data.map(
          (ministry) => new Ministry(ministry),
        );
                    ...
                    <li key={ministry._id || ministry.id}>
                      <button
                        className={`w-full text-left px-4 py-3 rounded-md flex items-center ${
                          activeMinistry === (ministry._id || ministry.id)
                            ? "bg-[#FFF5F0] text-[#FF7E45]"
                            : "hover:bg-gray-50"
                        }`}
                        onClick={() =>
                          setActiveMinistry(ministry._id || ministry.id)
                        }
                      >
                        <i
                      ...
                      <>
                        ...
                        <div className="flex flex-wrap gap-2 mb-8">
                          {currentMinistry.volunteerNeeds.map((need, idx) => (
                            <span
                              key={idx}
                              className="bg-[#FFF5F0] text-[#FF7E45] px-3 py-1 rounded-full text-sm"
                            >
                              {need}
                            </span>
                          ))}
