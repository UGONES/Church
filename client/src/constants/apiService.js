...
export const ministryService = {
  ...
  getVolunteerOpportunities: () =>
    apiClient.get(FRONTEND_ENDPOINTS.PUBLIC.MINISTRIES_VOLUNTEER),
  getUserMinistries: () =>
    apiClient.get(FRONTEND_ENDPOINTS.PUBLIC.MINISTRIES_USER),
  volunteer: (id, formData) =>
    apiClient.post(
      FRONTEND_ENDPOINTS.PUBLIC.MINISTRIES_VOLUNTEER_ACTION(id),
      formData,
    ),
  contactLeaders: (id, message) =>
    apiClient.post(FRONTEND_ENDPOINTS.PUBLIC.MINISTRIES_CONTACT(id), {
      message,
    }),
...
export const sermonService = {
  ...
  getFeatured: (limit = 3) =>
    apiClient.get(
      `${FRONTEND_ENDPOINTS.PUBLIC.SERMONS_FEATURED}&limit=${limit}`,
    ),
  ...
  addFavorite: (sermonId) =>
    apiClient.post(
      FRONTEND_ENDPOINTS.PUBLIC.SERMONS_FAVORITE_ACTION(sermonId),
      { sermonId },
    ),
...
export const eventService = {
  ...
  getUpcoming: (limit = 3) =>
    apiClient.get(
      `${FRONTEND_ENDPOINTS.PUBLIC.EVENTS_UPCOMING}&limit=${limit}`,
    ),
...
export const testimonialService = {
  ...
  getApproved: (limit = 6) =>
    apiClient.get(
      `${FRONTEND_ENDPOINTS.PUBLIC.TESTIMONIALS_APPROVED}&limit=${limit}`,
    ),
  ...
  create: (testimonialData) =>
    apiClient.post(
      FRONTEND_ENDPOINTS.ADMIN.TESTIMONIALS.CREATE,
      testimonialData,
    ),
  update: (id, testimonialData) =>
    apiClient.put(
      FRONTEND_ENDPOINTS.ADMIN.TESTIMONIALS.UPDATE(id),
      testimonialData,
    ),
  delete: (id) =>
...
export const prayerService = {
  ...
  getAll: (page = 1, limit = 10) =>
    apiClient.get(
      `${FRONTEND_ENDPOINTS.PUBLIC.PRAYERS}?page=${page}&limit=${limit}`,
    ),
...
export const donationService = {
  ...
  downloadReceipt: (donationId) =>
    apiClient.get(FRONTEND_ENDPOINTS.ADMIN.DONATIONS.RECEIPT(donationId), {
      responseType: "blob",
    }),
  ...
  exportDonations: (format = "csv") =>
    apiClient.get(
      `${FRONTEND_ENDPOINTS.ADMIN.DONATIONS.EXPORT}?format=${format}`,
      { responseType: "blob" },
    ),
};
...
export const authService = {
  ...
  socialLogin: (provider) =>
    (window.location.href = AUTH_ENDPOINTS.SOCIAL_LOGIN(provider)),
};
