...
const configurePassport = () => {
  ...
  if (!envVarsValid) {
    console.warn(
      "⚠️ Social authentication disabled due to missing environment variables",
    );
    return;
  }
  ...
  if (process.env.FACEBOOK_APP_ID && process.env.FACEBOOK_APP_SECRET) {
    ...
    console.warn(
      "❌ Facebook OAuth not configured - missing environment variables",
    );
  }
  ...
  router.get(
    ...
    (req, res) => {
      try {
        ...
        res.redirect(
          `${process.env.CLIENT_URL || "http://localhost:5173"}/auth/success?token=${token}&userId=${req.user._id}`,
        );
      } catch (error) {
