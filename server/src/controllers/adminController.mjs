...
export async function getAdminCodes(req, res) {
  try {
    ...
    const docs = await AdminCode.find(query)
...
export async function updateUserRole(req, res) {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ success: false, message: "Cannot demote yourself" });
    }
    ...
    return res.json({
      ...
      user: updated.getPublicProfile ? updated.getPublicProfile() : updated,
    });
  } catch (err) {
...
export async function getDashboardStats(req, res) {
  try {
    ...
    let recentActivity = [];
    try {
      recentActivity = await Analytics.find()
